...
for iStep = 1:nGDSteps
    ...
    % TODO: remove this
    % % store HP values at this GD step
    % for iHP = 1:size(hps, 1)
    %     hpRecord(iStep, iHP) = modelNeuronObj.dendParams.(hps{iHP});
    %     % fprintf(hpFileID, "%f,", modelNeuronObj.dendParams.(hps{iHP}));
    % end
    % % fprintf(hpFileID, "\n");
...
%% DEBUG: ALL NANS IN LOSS FUNCTION
modelNeuronObj = modelInit(seedlist(iStep));
for iHP = 1:size(hps, 1)
    modelNeuronObj.dendParams.(hps{iHP}) = hpRecord(iStep, iHP);
end
...
J_test = calcLoss(modelNeuronObj, resultsRFAfter);
