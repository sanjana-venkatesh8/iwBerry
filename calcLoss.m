...
function [totalLoss, perBranchLoss, somaLoss] = calcLoss(mN, resultsRFAfter)
    ...
    perBranchLossRFSize = 1/(fieldSize) .* (fieldSize - resultsRFAfter.branchSize1(1:mN.dendParams.nBranches));
    
    perBranchLoss = 2 * perBranchLossIOrient + (1/64) * perBranchLossRFSize;
    ...
    somaLoss = 2 * resultsRFAfter.branchIOrient(iSoma) + (1/64) * resultsRFAfter.branchSize1(iSoma);
end